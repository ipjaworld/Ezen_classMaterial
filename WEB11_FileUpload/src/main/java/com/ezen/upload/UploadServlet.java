...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. 파일이 업로드될 타겟폴더를 지정, String 으로 저장해둡니다.
		String savePath = "upload";
		
		// 2. 업로드될 파일의 용량을 제한하기 위한 용량값을 int 변수에 저장해둡니다.
		int uploadFileSizeLimit = 5 * 1024 * 1024;
		// 1바이트 기준 1024byte = 12Kbyte
		// 1024KByte = 1MB
		// 1024MByte = 1GB
		// 1024GByte = 1TB
		// 1024TByte = 1PB
		...
		MultipartRequest multi = new MultipartRequest(
				...
				// enctype="multipart/form-data"으로 전달된 폼의 모든 파라미터들은 일반 request.getParameter() 로 값을 얻을 수 없습니다
				// MultipartRequest 에 request 를 넣어서 복합 사용해야 담겨있는 유효한 값을 얻을 수 있습니다.
				// 결론적으로 multipart/form-data 로 보낸 데이터들은 MultipartRequest로만 받을 수 있습니다.
				// multipart/form-data 로 보낸 데이터들을 일반 request.getParameter 로 받으면 받은 값이 모두 null이 됩니다.
				
				uploadFilePath,	// 서버 상의 실제 저장장소
		...
		// 일반 request 는 multipart/form-data 로 전달된 파라미터가 모두 null 로 수신됩니다.
