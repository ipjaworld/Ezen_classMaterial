...
public class OrderDao {
	...
	public int insertOrder(ArrayList<CartVO> cartList, String id) {
		...
		try {
			...
			// => 늘어나는 시퀀스 값으로 oseq가 입력되었을 것이므로 가장 큰 oseq 값이 방금 추가된 값이 됩니다.
			sql = "select max(oseq) as max_oseq from orders";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if( rs.next() ) oseq = rs.getInt("max_oseq"); 
			
			// 3. 카트목록과 방금 조회한 가장 큰 oseq를 이용해서 order_detail 에 레코드를 카트 상품 단위로 하나씩 추가합니다
			...
			// 4. order_detail 에 추가된 카트 내용은 cart 테이블에서 처리되었으므로 삭제 또는 result를 2로 변경 또는 삭제
