...
public class CalendarCalss01 {

	public static void main(String[] args) {
		
		Calendar today = Calendar.getInstance();  // 현재날짜는 오늘날짜
		// 싱글톤 방식의 클래스.  
		// 생성자가 private 으로 접근 차단됨(외부에서 접근 금지.->외부에서 new Calendar() 사용 안됨)
		// 유일하게 실행된 new Calendar() 는 클래스 내부에서 실행되어 클래스내부에 단하나만 존재합니다
		// 그리고 그 레퍼런스 변수가 갖고 있는 주소를 리턴하는 public static 형태의 getInstance 메서드로  
		// 인스턴스의 주소를 리턴 받아 씁니다.
		// 이후 다시 getInstance() 메서드가 실행되도 현재 HEAP 에 이미 있는 인스턴스의 주소가 리턴 됩니다.
		Calendar tomorrow = Calendar.getInstance();    // 특별히 날짜 지정을 하지 않으면 현재날짜는 오늘날짜 
		// 공유된 인스턴스에서  현재날짜에 관한 정보만  객체끼지 서로 다른 날짜를 따로 관리하고 많은 내용이 공유됩니다.
		...
		// 1을 전달하면 연도를 추출합니다.
		// String 으로 만든 날짜 : "2022-10-17"
		// Date 또는 Calendar 로 만든 날짜 : 2022-10-17
		// String 데이터에서 2022를 추출하려면 substring 사용 -> 결과 String
		// Calendar에서 2022를 추출하려면 get 사용  -> 결과 int
		
		System.out.println("년도 : " + today.get(Calendar.YEAR));
		// 연도에 필요한 전달인수 1 대신에  Calendar.YEAR 를 사용해서 연도 추출
