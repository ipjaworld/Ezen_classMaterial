...
-- 이미 만들어져 있는 테이블의 구조 등을 수정할때 사용합니다.
...
-- ●● create 로 테이블을 만들며 생성한 기본키와 외래키 등등은 필드명이 바뀌어도 바뀐 이름으로 자동 적용됩니다
...
-- 필드LEVEL(수준) 의 제약조건은 필드를 modify 하여서 같이 수정 생성 합니다.
-- 테이블 LEVEL 의 제약 조건은 위의 명령 형식으로 제약조건 이름과 함께 추가합니다

-- memberlist 테이블의 성별 (gender) 필드에 'F', 'M' 두글자만 입력되도록 제약조건을 추가하세요
-- alter table memberlist add constraint check( gender='F' or gender='M' );
...
--    gender 필드의 들아갈 값이 in( ) 함수 안의 항목중 하나와 같다면 입력 허용, 아니면 불허하는 제약조건입니다
...
alter table rentlist modify membernum number(5);
dk 
-- 위에 삭제되었던 fk1, fk2, rt_pk 제약조건은 다시 설정하세요
...
alter table rentlist add constraint rt_pk primary key( rentdate, numseq );
